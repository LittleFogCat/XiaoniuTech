    ...
    <Select.Root value={value} onValueChange={onChange}>
      ...
      <Select.Portal>
        <Select.Content
          ...
          sideOffset={6}
          collisionPadding={8}
          className="z-50 min-w-[var(--radix-select-trigger-width)] w-max max-w-[min(92vw,30rem)] overflow-hidden rounded-[22px] border border-slate-700/70 bg-[linear-gradient(180deg,rgba(30,41,59,0.96),rgba(15,23,42,0.98))] shadow-[0_18px_48px_rgba(15,23,42,0.28)] backdrop-blur-xl"
        >
          <Select.Viewport className="p-2">
            {models.map((model) => (
              <Select.Item
                key={model.id}
                value={model.id}
                className="relative flex min-h-11 cursor-pointer select-none items-center gap-3 whitespace-nowrap rounded-2xl px-3 py-2.5 text-sm text-slate-200 outline-none transition data-[highlighted]:bg-sky-500/12 data-[highlighted]:text-white data-[state=checked]:bg-sky-500/10 sm:px-4"
              >
